 ...
 * Paths are resolved relative to the server root. The server must be
 * started from the docs/ directory (e.g. `npx serve docs/` or `python -m http.server` from docs/).
...
async function loadData() {
  const [jsonlRes, charterRes, deliverablesRes, issuesRes] = await Promise.allSettled([
    fetchRequired('/reports/change_and_decision_log.jsonl'),
    fetchRequired('/project_charter.md'),
    fetch('/deliverables.md').then(r => r.ok ? r.text() : ''),
    fetch('/reports/issues.jsonl').then(r => r.ok ? r.text().then(t => parseJsonl(t)) : []),
  ]);
...
function renderDeliverables(deliverables, issues) {
  ...
  const STATUS_LABEL = {
    ...
    closed: 'Complete', deferred: 'Deferred', under_review: 'Under Review',
  };
