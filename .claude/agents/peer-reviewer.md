...
The artifact under review is specified in the invocation. If no artifact is named, read `docs/project_charter.md` to orient yourself, then ask the user to specify what should be reviewed. The project's grounding references, if any, are in `docs/background.md`.
...
After completing the review, write it to `docs/critiques/`. Create the directory if it does not exist. Name the file using the pattern `YYYY-MM-DD_[artifact-slug]_[mode].md`, where:
...
Example: `docs/critiques/2026-04-05_project-charter_section.md`
...
Read the specified artifact to locate the claim and its citations. To assess the cited sources, read `docs/bibliography/index.md` to identify the relevant entry files, then read the specific individual files in `docs/bibliography/` for each cited source.

Assess on these dimensions:

1. **Claim** — Quote the claim as stated in the paper, with section attribution.
2. **Citation accuracy** — Does each cited source actually support the claim as stated? A related source that does not support the specific claim is insufficient. Quote the Project Relevance section from the relevant bibliography entry file.
