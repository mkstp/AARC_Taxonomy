"""
MiPROv2 optimization for 3-label (A/G/R) AARC turn annotation.

Compiles an AGRAnnotator DSPy program against the 140-turn gold set using a
recall-weighted metric: tp=1.0, tn=0.3, fp/fn=0.0. Clarity is excluded from
both the signature and the metric — its failure is a taxonomy grounding problem
deferred to a future pass (see FPO-aq6).

Defaults to medium-tier search with val-size=70 (larger held-out set than the
prior 4-label light runs, which used val-size=40).

Each run saves to a timestamped file (data/dspy_program_ara_YYYYMMDD_HHMMSS.json)
so prior programs are never overwritten. Pass the saved path to silver_annotate
via --program-path --provider dspy-ara.

Usage:
    python3 scripts/optimize_dspy_ara.py                          # medium run, gpt-5.4
    python3 scripts/optimize_dspy_ara.py --auto light
    python3 scripts/optimize_dspy_ara.py --eval-only --program data/dspy_program_ara_<ts>.json
"""
...
from fpo_mediation.dspy_annotator_ara import AGRAnnotator, LABELS
...
def print_report(report: dict, label: str = "") -> None:
    ...
    if header:
        print(header)
    short = {
        "acknowledgement_deficit": "Ack",
        "agency_deficit":          "Aga",
        "reciprocity_deficit":     "Rec",
    }
    for l in LABELS:
        r = report[l]
    ...
    # Baseline comparison (GPT-5.4 original → Claude target)
    baselines = {
        "acknowledgement_deficit": {"gpt_f1": 0.633, "claude_f1": 0.824},
        "agency_deficit":          {"gpt_f1": 0.435, "claude_f1": 0.649},
        "reciprocity_deficit":     {"gpt_f1": 0.595, "claude_f1": 0.812},
    }
    print()
    print("    Baseline comparison:")
    print(f"    {'Label':<8}  {'This run':>8}  {'GPT-5.4':>8}  {'Claude':>8}  {'vs GPT':>8}  {'vs Claude':>10}")
    for l in LABELS:
        r = report[l]
        b = baselines[l]
        vs_gpt = r["f1"] - b["gpt_f1"]
        vs_claude = r["f1"] - b["claude_f1"]
        print(f"    {short[l]:<8}  {r['f1']:>8.3f}  {b['gpt_f1']:>8.3f}  {b['claude_f1']:>8.3f}  {vs_gpt:>+8.3f}  {vs_claude:>+10.3f}")


def main() -> None:
    parser = argparse.ArgumentParser(description="MiPROv2 optimization for AGR (3-label) AARC annotation.")
    ...
    parser.add_argument("--auto", default="medium", choices=["light", "medium", "heavy"],
                        help="MiPROv2 search budget (default: medium)")
    ...
    parser.add_argument("--val-size", type=int, default=70,
                        help="Number of gold examples held out for validation (default: 70)")
    parser.add_argument("--seed", type=int, default=42)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    parser.add_argument("--output", default=str(ROOT / "data" / f"dspy_program_ara_{ts}.json"))
